            ...
            foreach ($query_response->ENTITY as $entity) {
                if ($entity['entityspec'] == 'AuthRole') {
                    foreach ($entity->PROPERTY as $key) {
                        if ($key['path'] == 'name')
                            $user['authrole'][] = (String) $key['value'];
                    }
                }
                if ($entity['entityspec'] == 'LocalePreference') {
                    foreach ($entity->PROPERTY as $key) {
                        if ($key['path'] == 'locality.code')
                            $user['locale'][] = (String) $key['value'];
                    }
                }
                if ($entity['entityspec'] == 'UserAuthInfo') {
                    foreach ($entity->PROPERTY as $key) {
                        if ($key['path'] == 'password')
                            $user['user_hash'][] = (String) $key['value'];
                    }
                }
                if ($entity['entityspec'] == 'Person') {
                    foreach ($entity->PROPERTY as $key) {
                        if ($key['path'] == 'firstName')
                            $user['user_nm']['firstName'] = (String) $key['value'];
                        if ($key['path'] == 'lastName')
                            $user['user_nm']['lastName'] = (String) $key['value'];
                    }
                }
            }
            return $user;
