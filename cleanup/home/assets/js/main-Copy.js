		...
		if (e.keyCode == 13) {
			...
			$.ajax({
					"type":"POST", "url":"assets/common/ajax.inc.php", "data":$(this).serialize(), "success":function(data){
						...
						countChecked(5); paginate(); doMerge(); zebraTable();
					}
			});
	...
	$('a.clean_cust').on('click', function(e){
		...
		$.ajax({
				"type":"POST", "url":"assets/common/ajax.inc.php", "data":data, "success":function(data){
					$('#search_results').html(data);
					zebraTable(); comboBox(); openClsInputs(); validate();					
				}
		});
	});
}
	...
	$('a.preview').on('click', function(e){
		...
		if( valid ){
			...
			$.ajax({
				"type":"POST", "url":"assets/common/ajax.inc.php", "data":data, "success":function(data){
					$('#dialog-confirm').html(data);
					var lastUrlPart = window.location.href.split("/").pop()
...
var validate = function(){
	$("input[name=PRIMARYPHONENO_]").change(function() { isPhone($("input[name=PRIMARYPHONENO_]:not(:disabled)"), 'PRIMARYPHONENO_', "Primary Phone invalid, pls check input", true)});
	$("input[name=WORKPHONENUMBER_]").change(function() { isPhone($("input[name=WORKPHONENUMBER_]:not(:disabled)"), 'WORKPHONENUMBER_', "Work Phone invalid, pls check input", true)});
	$("input[name=PRIMARYEMAILADDRESS_]").change(function() { isEmailValid($("input[name=PRIMARYEMAILADDRESS_]:not(:disabled)"), 'PRIMARYEMAILADDRESS_', "Primary email address invalid, pls check input", true)});
}
...
function validateCoporate(){
  if(validName($("input[name=FULLNAME_]:not(:disabled)"), 'FIRSTNAME_',  "Customer name you entered is incorrect, pls check input")){
