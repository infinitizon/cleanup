...
$tblTitle = array('sn' => 'S/N', 'PARTYROLE_ID' => 'Party Role ID.', 'PARTY_ID' => 'Party ID.', 'PARTYTITLE_ID' => 'Title'
    , 'CUSTOMERID_' => 'ERP Customer ID', 'LEGACYSTCUSTOMERID_' => 'iBroker ST Customer ID', 'LEGACYIMCUSTOMERID_' => 'iBroker IM Customer ID', 'ERPST_ACC_NOS' => 'ST Account No.'
    , 'LEGACYSTACCOUNTNUMBER_' => 'iBroker ST Account #', 'LEGACYMMACCOUNTNUMBER_' => 'iBroker MM Account #'
    , 'LEGACYTBILLSACCOUNTNUMBER_' => 'iBroker TBills Account #', 'TOTALASSETVALUE_' => 'Total Asset Value (NGN)', 'TYPE_ID' => 'Type ID.'
    , 'FIRSTNAME_' => 'First Name', 'MIDDLENAME_' => 'Middle Name', 'LASTNAME_' => 'Last Name', 'INITIALS_' => 'Initials', 'GENDER_ID' => 'Gender'
    , 'MARITALSTATUSTYPE_ID' => 'Marital Status', 'MAIDENNAME_' => 'Maiden Name', 'DATEOFBIRTH_' => 'Date Of Birth'
    , 'PRIMARYPHONENO_' => 'Phone No. (Mobile)', 'WORKPHONENUMBER_' => 'Phone No. (Other)', 'PRIMARYEMAILADDRESS_' => 'Email Address'
    , 'ADDRESSLINE1_' => 'Address Line 1', 'ADDRESSLINE2_' => 'Address Line 2', 'ADDRESSCITY_' => 'City', 'ADDRESSCOUNTRY_ID' => 'Country'
    , 'ADDRESSSTATE_ID' => 'State', 'COUNTRY_ID' => 'Nationality', 'KINRELATIONSHIPTYPE_ID' => 'Next Of Kin Type', 'KINFIRSTNAME_' => 'Kin First Name'
...
if (@$_POST['cust_type'] == '4') {
    $tblTitle = array('PARTYROLE_ID' => 'Party Role ID.', 'PARTY_ID' => 'Party ID.', 'PARTYTITLE_ID' => 'Title', 'CUSTOMERID_' => 'ERP Customer ID'
        , 'LEGACYSTCUSTOMERID_' => 'iBroker ST Customer ID', 'LEGACYIMCUSTOMERID_' => 'iBroker IM Customer ID', 'ERPST_ACC_NOS' => 'ST Account No.', 'LEGACYSTACCOUNTNUMBER_' => 'iBroker ST Account #'
        , 'LEGACYMMACCOUNTNUMBER_' => 'iBroker MM Account #', 'LEGACYTBILLSACCOUNTNUMBER_' => 'iBroker TBills Account #'
...
if (isset($_POST['get_merger'])) {
    $surviving = "SELECT ms.sn, ms.PARTYROLE_ID, ms.PARTY_ID, ms.CUSTOMERID_, ms.TYPE_ID, ms.ERPST_ACC_NOS"
            . ", ms.ERPIMACCOUNTNUMBER_, ms.LEGACYSTACCOUNTNUMBER_, ms.LEGACYMMACCOUNTNUMBER_"
            . ", ms.LEGACYTBILLSACCOUNTNUMBER_, ms.LEGACYSTCUSTOMERID_, ms.LEGACYIMCUSTOMERID_, ms.PARTYTITLE_ID, ms.FIRSTNAME_"
            . ", ms.MIDDLENAME_, ms.LASTNAME_, ms.INITIALS_, ms.GENDER_ID, ms.MARITALSTATUSTYPE_ID, ms.MAIDENNAME_"
            . ", DATE_FORMAT(ms.DATEOFBIRTH_, '%b-%d-%Y') AS DATEOFBIRTH_, ms.PRIMARYEMAILADDRESS_, ms.COUNTRY_ID"
            . ", ms.PRIMARYPHONENO_, ms.WORKPHONENUMBER_, ms.ADDRESSLINE1_, ms.ADDRESSLINE2_, ms.ADDRESSCITY_"
            . ", ms.ADDRESSSTATE_ID, ms.ADDRESSCOUNTRY_ID, ms.KINRELATIONSHIPTYPE_ID, ms.KINFIRSTNAME_"
            . ", ms.KINLASTNAME_, ms.KINOTHERNAMES_, KINCONTACTPHONENUMBER_, ms.KINADDRESS_, ms.LOCALE_, ms.FULLNAME_"
            . ", (SELECT p.TOTALASSETVALUE_ FROM PARTYROLE_ p WHERE p.PARTYROLE_ID=ms.PARTYROLE_ID) TOTALASSETVALUE_"
            . ", ms.DUPLICATE_IDS FROM migrationstatus_ ms WHERE sn=" . $_POST['id'];
    if($_POST['approveTp']==1){
        $surviving = "SELECT ms.sn, m.PARTYROLE_ID, m.PARTY_ID, m.CUSTOMERID_, m.TYPE_ID, m.ERPST_ACC_NOS"
            . ", m.ERPIMACCOUNTNUMBER_, m.LEGACYSTACCOUNTNUMBER_, m.LEGACYMMACCOUNTNUMBER_"
            . ", m.LEGACYTBILLSACCOUNTNUMBER_, m.LEGACYSTCUSTOMERID_, m.LEGACYIMCUSTOMERID_, m.PARTYTITLE_ID, m.FIRSTNAME_"
            . ", m.MIDDLENAME_, m.LASTNAME_, m.INITIALS_, m.GENDER_ID, m.MARITALSTATUSTYPE_ID, m.MAIDENNAME_"
            . ", DATE_FORMAT(m.DATEOFBIRTH_, '%b-%d-%Y') AS DATEOFBIRTH_, m.PRIMARYEMAILADDRESS_, m.COUNTRY_ID"
            . ", m.PRIMARYPHONENO_, m.WORKPHONENUMBER_, m.ADDRESSLINE1_, m.ADDRESSLINE2_, m.ADDRESSCITY_"
            . ", m.ADDRESSSTATE_ID, m.ADDRESSCOUNTRY_ID, m.KINRELATIONSHIPTYPE_ID, m.KINFIRSTNAME_"
            . ", m.KINLASTNAME_, m.KINOTHERNAMES_, m.KINCONTACTPHONENUMBER_, m.KINADDRESS_, m.LOCALE_, m.FULLNAME_"
            . ", (SELECT p.TOTALASSETVALUE_ FROM PARTYROLE_ p WHERE p.PARTYROLE_ID=m.PARTYROLE_ID) TOTALASSETVALUE_"
            . ", ms.DUPLICATE_IDS "
            . " FROM migrationstatus_ ms JOIN MIGRATION_ m ON m.PARTYROLE_ID =  ms.PARTYROLE_ID"
            . " WHERE ms.sn =" . $_POST['id'];    
    }
    $surviving = $fxns->_execQuery($surviving, true, false);
    if ($surviving['TYPE_ID'] == 4) {
        $tblTitle = array('sn' => 'S/N', 'PARTYROLE_ID' => 'Party Role ID.', 'PARTY_ID' => 'Party ID.', 'PARTYTITLE_ID' => 'Title'
            , 'CUSTOMERID_' => 'ERP Customer ID', 'LEGACYSTCUSTOMERID_' => 'iBroker ST Customer ID', 'LEGACYIMCUSTOMERID_' => 'iBroker IM Customer ID'
            , 'ERPST_ACC_NOS' => 'ST Account No.', 'LEGACYSTACCOUNTNUMBER_' => 'iBroker ST Account #', 'LEGACYMMACCOUNTNUMBER_' => 'iBroker MM Account #'
            , 'LEGACYTBILLSACCOUNTNUMBER_' => 'iBroker TBills Account #', 'TOTALASSETVALUE_' => 'Total Asset Value (NGN)', 'TYPE_ID' => 'Type ID.'
            , 'FULLNAME_' => 'Name', 'PRIMARYPHONENO_' => 'Contact Phone Number', 'WORKPHONENUMBER_' => 'Phone No. (Other)'
            , 'PRIMARYEMAILADDRESS_' => 'Email Address', 'ADDRESSLINE1_' => 'Address Line 1', 'ADDRESSLINE2_' => 'Address Line 2'
            , 'ADDRESSCITY_' => 'City', 'ADDRESSCOUNTRY_ID' => 'Country', 'ADDRESSSTATE_ID' => 'State');
        if (($key = array_search('FULLNAME_', $hiddenFields)) !== false) {
            unset($hiddenFields[$key]);
        }
    }
    $dupIDs = str_replace(',', "','", $surviving['DUPLICATE_IDS']);
    $others = "SELECT '' sn, m.PARTYROLE_ID, m.PARTY_ID, m.CUSTOMERID_, m.TYPE_ID, m.ERPST_ACC_NOS"
            . ", m.ERPIMACCOUNTNUMBER_, m.LEGACYSTACCOUNTNUMBER_, m.LEGACYMMACCOUNTNUMBER_"
            . ", m.LEGACYTBILLSACCOUNTNUMBER_, m.LEGACYSTCUSTOMERID_, m.LEGACYIMCUSTOMERID_, m.PARTYTITLE_ID, m.FIRSTNAME_, m.MIDDLENAME_"
            . ", m.LASTNAME_, m.INITIALS_, m.GENDER_ID, m.MARITALSTATUSTYPE_ID, m.MAIDENNAME_"
            . ", DATE_FORMAT(m.DATEOFBIRTH_, '%b-%d-%Y') AS DATEOFBIRTH_, m.PRIMARYEMAILADDRESS_, m.COUNTRY_ID"
            . ", m.PRIMARYPHONENO_, m.WORKPHONENUMBER_, m.ADDRESSLINE1_, m.ADDRESSLINE2_, m.ADDRESSCITY_"
            . ", m.ADDRESSSTATE_ID, m.ADDRESSCOUNTRY_ID, m.KINRELATIONSHIPTYPE_ID, m.KINFIRSTNAME_, m.KINLASTNAME_"
            . ", m.KINOTHERNAMES_, m.KINCONTACTPHONENUMBER_, m.KINADDRESS_, m.LOCALE_, m.FULLNAME_"
            . ", (SELECT p.TOTALASSETVALUE_ FROM PARTYROLE_ p WHERE p.PARTYROLE_ID = m.PARTYROLE_ID) TOTALASSETVALUE_ "
            . "FROM migration_ m WHERE PARTYROLE_ID IN ('" . $dupIDs . "')";

    $all4Pending = $fxns->_execQuery($others, true, true);
    array_unshift($all4Pending, $surviving);
    array_unshift($all4Pending, $tblTitle);
    //print_r($all4Pending);exit;
    ...
    foreach ($all4Pending[0] as $key => $values) {
        ...
        if($_POST['approveTp']==1){
            $sql_getComboSTAT = " UNION ALL " . $sql_getCombo . "FROM MIGRATION_ WHERE PARTYROLE_ID IN('{$surviving['PARTYROLE_ID']}')";
        }else{
	...
	WHERE TYPE_ID = (SELECT GEOGRAPHICBOUNDARYTYPE_ID FROM GEOGRAPHICBOUNDARYTYPE_ WHERE NAME_ = 'Country') ORDER BY NAME_";
                        ...
                        if ($key == 'TOTALASSETVALUE_')
                            $table .= '<td><div style="width:200px; text-align:right;">' . $fxns->_formatMoney($all4Pending[$i][$key], true) . '</div></td>';
                        else
                            $table .= "<td>" . $all4Pending[$i][$key] . '<input type="hidden" name="' . $key . '" value="' . $all4Pending[$i][$key] . '" /></td>';
                    }else {
    ...
    $table .= "</table>";
    if($_POST['approveTp']!=1){  //Check if we are NOT coming from approved screen
...
if (isset($_POST['rjtRsnId'])) {
    $sql_rjtRsn = "SELECT REJECTREASON_ FROM MIGRATIONSTATUS_ WHERE sn={$_POST['rjtRsnId']}";
    $rjtRsn = $fxns->_execQuery($sql_rjtRsn, true, false);
    echo 'Reason: '.$rjtRsn['REJECTREASON_'];
}
if (isset($_POST['rsn'])) {
    $sn = array_filter($_POST['sn']);
    $sql_rejectMerge = "UPDATE MIGRATIONSTATUS_ SET STATUS_=2, REJECTREASON_='".mysql_real_escape_string($_POST['rsn'])."'";
    $sql_rejectMerge .= ", APPROVEDBY_='{$_SESSION['user']}', APPROVEDATE_=NOW() WHERE sn = {$sn[0]}";
...
if (isset($_POST['mergeNow'])) {
    ...
    if (in_array("MIGRATION_OFFICER", $_SESSION['user_dets']['authrole'])) {
        ...
        $partyRole = ($_POST['cust_type'] == 4) 
                ? array('CUSTOMERID_') : array('CUSTOMERID_', 'KINRELATIONSHIPTYPE_ID', 'KINFIRSTNAME_', 'KINLASTNAME_', 'KINOTHERNAMES_'
                    ,'KINCONTACTPHONENUMBER_', 'KINADDRESS_', 'LEGACYSTCUSTOMERID_', 'LEGACYIMCUSTOMERID_');
        $party = ($_POST['cust_type'] == 4) 
                ? array('PARTYTITLE_ID', 'PRIMARYEMAILADDRESS_', 'PRIMARYPHONENO_', 'WORKPHONENUMBER_', 'ADDRESSLINE1_','ADDRESSLINE2_'
                    , 'ADDRESSCITY_', 'ADDRESSCOUNTRY_ID', 'ADDRESSSTATE_ID', 'FULLNAME_') 
                : array('PARTYTITLE_ID', 'FIRSTNAME_', 'MIDDLENAME_', 'LASTNAME_', 'INITIALS_', 'GENDER_ID', 'MARITALSTATUSTYPE_ID','MAIDENNAME_'
                    , 'DATEOFBIRTH_', 'PRIMARYEMAILADDRESS_', 'COUNTRY_ID', 'PRIMARYPHONENO_', 'WORKPHONENUMBER_', 'ADDRESSLINE1_', 'ADDRESSLINE2_'
                    , 'ADDRESSCITY_', 'ADDRESSCOUNTRY_ID', 'ADDRESSSTATE_ID', 'FULLNAME_');
        ...
        foreach ($_POST as $key => $value) {
            if ($key != 'checkIDs' && $key != 'mergeNow' && $key != 'ERPST_ACC_NOS' && $key != 'cust_type') {
                $sql_merge .= ($key == 'DATEOFBIRTH_') ? " $key=" . ((empty($value) || $value ='') ? "NULL," : "'" . date("Y-m-d H:i:s", strtotime($value)) . "',") : " $key = '$value', ";
            }
        }
        $nmCln = ((@$_POST['FIRSTNAME_'] != '' && strlen(@$_POST['FIRSTNAME_']) > 3 && @$_POST['LASTNAME_'] != '' && strlen(@$_POST['LASTNAME_']) > 3) || (isset($_POST['FULLNAME_']))) ? 1 : 0;
        ...
        $custMap = ($_POST['LEGACYSTCUSTOMERID_'] != '') ? 1 : 0;
        ...
        $sql_updt_iSTacc = "UPDATE ACCOUNT_ SET `BROKERAGENUMBER_` = ".(empty($_POST['LEGACYSTACCOUNTNUMBER_'])?"NULL":"'".$_POST['LEGACYSTACCOUNTNUMBER_']."'");
        $sql_updt_iSTacc .= " WHERE `CUSTOMER_PORTFOLIO_ID` = {$_POST['PARTYROLE_ID']} AND PRODUCT_ID IN ('655949824', '655949333')";

        if(!empty($_POST['LEGACYMMACCOUNTNUMBER_']) || !empty($_POST['LEGACYTBILLSACCOUNTNUMBER_'])){
            $chkIfAccExsts = "SELECT COUNT(*) count FROM ACCOUNT_ WHERE PRODUCT_ID='655949832' AND CUSTOMER_PORTFOLIO_ID={$_POST['PARTYROLE_ID']}";
            $hwMany = $fxns->_execQuery($chkIfAccExsts, TRUE, FALSE);
            if($hwMany['count'] == 0){
                $cust->_createCustAcc($_POST['PARTYROLE_ID'], '655949832');
            }
        }
        $sql_updt_iMMacc = "UPDATE ACCOUNT_ SET `MONEYMARKETNUMBER_`=".(empty($_POST['LEGACYMMACCOUNTNUMBER_'])?"NULL":"'".$_POST['LEGACYMMACCOUNTNUMBER_']."'")
                            . ",`TREASURYBILLNUMBER_`=".(empty($_POST['LEGACYTBILLSACCOUNTNUMBER_'])?"NULL":"'".$_POST['LEGACYTBILLSACCOUNTNUMBER_']."'");
        $sql_updt_iMMacc .= " WHERE `CUSTOMER_PORTFOLIO_ID` = {$_POST['PARTYROLE_ID']} AND PRODUCT_ID IN ('655949832')";
        /*         * **********  party and partyrole updates ends here ********************* */
        /*         * **********  Update the migrationstatus to remove currently processed ********************* */
        $sql_updt_migratnStat = "UPDATE MIGRATIONSTATUS_ SET `STATUS_`='1', APPROVEDBY_='{$_SESSION['user']}', APPROVEDATE_=NOW()";
        ...
        $_SESSION['sql_updt_iSTacc'] = $sql_updt_iSTacc;
        $_SESSION['sql_updt_iMMacc'] = $sql_updt_iMMacc;
        ...
        /*         * ********* Debugging *************
          ...
          echo $_SESSION['sql_updt_iSTacc']." <br /><br />";
          echo $_SESSION['sql_updt_iMMacc']." <br /><br />";
        ...
        if (empty($toBMergedArr)) {
            ...
            if ($doCustMerge['result'] == 'Success')
                $message .= '<form method="post" action="" name="reviewScreen" style="float:left;"><input type="hidden" name="showOk" value="1" /><table><tr><td>Customer information edited successfully!<blockquote>Click on another line item to approve merge.</blockquote></td></tr></table></form>';
            else
                $message .= $doCustMerge['msg'];
        }
        $table = '';
        if (count($toBMergedArr) + 1 > 1) {
            $message .='<form method="post" action="" name="reviewScreen"><input type="hidden" name="showOk" value="0" />';
            ...
            $table .= '<p>ST Account Information on Selected Customer</p><table  class="my_tables">';
            $table .= "<tr>";
            for ($i = 0; $i < count($survivingCustomers); $i++) {
                $table .= "<tr>";
                foreach ($planeTxt as $values) {
                    $table .= ($i == 0) ? "<th>" . $survivingCustomers[$i][$values] . "</th>" : "<td style='text-align:center;'>" .( ($values=='TOTALASSETVALUE_')?'<div style="text-align:right;">'.$fxns->_formatMoney($survivingCustomers[$i][$values]).'</div>':$survivingCustomers[$i][$values] ). "</td>";
                }
                $table .= '</tr>';
            }
            ...
            for ($i = 0; $i < count($all4Pending); $i++) {
                ...
                foreach ($planeTxt as $values) {
                    $table .= ($i == 0) ? "<th>" . $all4Pending[$i][$values] . "</th>" : "<td style='text-align:center;'>" .( ($values=='TOTALASSETVALUE_')?'<div style="text-align:right;">'.$fxns->_formatMoney($all4Pending[$i][$values]).'</div>':$all4Pending[$i][$values] ). "</td>";
                }
                $table .= "</tr>";
            }
...
if (isset($_POST['survivor'])) {
    ...
    if ($doCustMerge['result'] == 'Success') {
        ...
        if ($error)
            $message .= "Error while trying to perform the account merge. Transaction rolled back.<br /><br />Contact admin to complete.";
        else
            $message .="<br />Accounts also merged successfully.<br /><br /><br />Click on another line item to approve merge.";
    }else {
        $message .= $doCustMerge['msg'];
    }
    echo '<form method="post" action="" name="reviewScreen" style="float:left;"><input type="hidden" name="showOk" value="0" /><table><tr><td>'.$message.'</td></tr></table></form>';
}
...
function doCustMerge() {
    ...
    try {
        ...
        $fxns->_execQuery($_SESSION['sql_updt_iSTacc'], false, false);
        $fxns->_execQuery($_SESSION['sql_updt_iMMacc'], false, false);
